(function () {
  ...
  function replaceDollarText(node) {
    // Currency display is now controlled by admin - no auto replacement
    return;
  }
  ...
  // The React bundle always renders "$" + price for every property.
  // We fetch the properties API to build a price-number → currency map,
  // then walk all text nodes and rewrite "$X,XXX" to the correct symbol.

  var priceMap = {};   // id  → { price, currency }
  var numMap   = {};   // num → currency  (reverse lookup by amount)
  var priceMapLoaded = false;

  function loadPriceMap() {
    if (priceMapLoaded) return Promise.resolve();
    return fetch('/api/properties', { credentials: 'include' })
      ...
      .then(function (list) {
        ...
        list.forEach(function (p) {
          ...
          var num = typeof p.price === 'number' ? p.price : parseFloat(p.price);
          priceMap[p.id] = { price: num, currency: cur };
          if (!isNaN(num)) numMap[num] = cur;
        });
      })
      .catch(function () { priceMapLoaded = true; });
  }

  function formatPrice(num, currency) {
    var formatted = isNaN(num) ? String(num) : num.toLocaleString();
    if (currency === '$' || currency === 'USD') return '$' + formatted;
    return 'ETB ' + formatted;
  }

  // Matches "$1,500" or "ETB 1,500" or "ETB1,500" — the full text of a node
  var PRICE_RE = /^[\s]*(?:\$|ETB\s*)(\d[\d,]*)[\s]*$/;

  function fixAllPriceText() {
    ...
    while ((node = walker.nextNode())) {
      ...
      if (!raw) continue;
      var m = raw.match(PRICE_RE);
      if (!m) continue;
      var num = parseFloat(m[1].replace(/,/g, ''));
      if (isNaN(num)) continue;
      // Look up currency; anything not in map stays ETB
      var cur = numMap.hasOwnProperty(num) ? numMap[num] : 'ETB';
      var correct = formatPrice(num, cur);
      if (raw.trim() !== correct) node.nodeValue = correct;
    }
  }

  var fixTimer = null;
  function scheduleFixPrices(delay) {
    clearTimeout(fixTimer);
    fixTimer = setTimeout(function () {
      loadPriceMap().then(fixAllPriceText);
    }, delay || 150);
  }

  // Run multiple times to catch React's async renders
  function runFixCycle() {
    [100, 600, 1500, 3000].forEach(function (d) {
      setTimeout(function () {
        loadPriceMap().then(fixAllPriceText);
      }, d);
    });
