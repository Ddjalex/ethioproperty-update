...
- Live voice-to-voice (`extensions/features.js`, `/api/ai/live`) switched from `models/gemini-2.5-flash-native-audio-preview-12-2025` to `models/gemini-3.1-flash-live-preview` (July 13, 2026) to fix poor Amharic speech quality — 3.1 has improved multilingual native audio output. Verified the new model accepts the setup config and returns `ready` over the WebSocket. This app never updates the system instruction mid-session, so 3.1's lack of mid-session instruction updates does not apply here.
