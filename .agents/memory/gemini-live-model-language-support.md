...
## Related: `realtimeInput.mediaChunks` is deprecated
Separately, Gemini Live's client→server audio-streaming message format changed: the older `realtimeInput: { mediaChunks: [{ mimeType, data }] }` (array) is deprecated and gets a 1007 close with reason "realtime_input.media_chunks is deprecated. Use audio, video, or text instead." The current format is a single object: `realtimeInput: { audio: { mimeType, data } }`. A working `setup`/`ready` handshake can mask this — the close only happens once real audio chunks start flowing, so test with an actual (even silent) PCM frame, not just the init handshake, when verifying a live-voice fix.
